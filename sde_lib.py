...
class VPSDE(SDE):
  ...
  def sde(self, observed_data, t):
    beta_t = self.beta_0 + t * (self.beta_1 - self.beta_0)
    drift = -0.5 * beta_t[:, None, None] * observed_data
    diffusion = torch.sqrt(beta_t)
    return drift, diffusion

  def marginal_prob(self, observed_data, t):
    log_mean_coeff = -0.25 * t ** 2 * (self.beta_1 - self.beta_0) - 0.5 * t * self.beta_0
    mean = torch.exp(log_mean_coeff[:, None, None]) * observed_data
